...
const ReportSection = () => {
  ...
  const {
    ...
    const getVehicleStats = () => {
        const vehicleStats: Record<string, { vehicle: string | undefined, type: string | undefined, status: string, bookings: number, revenue: number }> = {};
        getFilteredBookings().forEach(booking => {
            if (!booking.vehicleId) return; // Skip if no vehicle is assigned
    
            if (!vehicleStats[booking.vehicleId]) {
                vehicleStats[booking.vehicleId] = {
                    vehicle: `${booking.vehicle?.make} ${booking.vehicle?.model} ${booking.vehicle?.licensePlate}`,
                    type: booking?.vehicle?.type || "",
                    bookings: 0,
                    status: booking.vehicle?.availabilityStatus || "",
                    revenue: 0
                };
            }
    
            vehicleStats[booking.vehicleId].bookings += 1;
            vehicleStats[booking.vehicleId].revenue += Number(booking.totalPrice);
        });
    
        return Object.values(vehicleStats);
    }
    
    const getBookerStats = () => {
        const bookerStats: Record<string, { name: string, email:String, bookings: number, vehicles: Record<string, { vehicle: IVehicle | undefined, count: number }>, totalSpent: number }> = {};
    
        getFilteredBookings().forEach(booking => {
            const bookerId = booking.bookerId;
            ...
            if (vehicleId) {
                if (!bookerStats[bookerId].vehicles[vehicleId]) {
                    bookerStats[bookerId].vehicles[vehicleId] = {
                        vehicle: booking.vehicle,
                        count: 0
                    };
                }
                bookerStats[bookerId].vehicles[vehicleId].count += 1;
            }
            bookerStats[bookerId].bookings += 1;
            bookerStats[bookerId].totalSpent += Number(booking.totalPrice);
        });
    
        return Object.values(bookerStats);    
    }
    ...
    <Wrapper currentTab="reports">
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        {!isBooker && selectedView === "bookers" && <div className="shadow-lg rounded-lg bg-white p-4">
          ...
          <TableRecords 
            columns={["Name", "Email", "Bookings", "Total Spent"]}
            data={getBookerStats()}
            keys={["name", "email", "bookings", "totalSpent"]}
          />
        ...
        { selectedView === "vehicles" && <div className="shadow-lg rounded-lg bg-white p-4">
          ...
          <TableRecords 
            columns={["Vehicle", "Type", "Status", "Bookings", "Revenue"]}
            data={getVehicleStats()}
            keys={["vehicle", "type", "status", "bookings", "revenue"]}
          />
          {/* <div className="max-h-64 overflow-y-auto space-y-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
              ...
              <TableRecords
                  data={getFilteredBookings().map((b) => ({
                    booker: `${b.booker.firstName} ${b.booker.lastName}`,
                    vehicle: `${b.vehicle?.model} ${b.vehicle?.make}`  || "Unknown",
                    date: new Date(b.startDate).toLocaleDateString(),
                    duration: `${Math.ceil((new Date(b.endDate).getTime() - new Date(b.startDate).getTime()) / (1000 * 60 * 60 * 24))} Days`,
                    cost: formatMoney(b.totalPrice),
                    status: b.status,
                  }))}
                  columns={["Booker", "Vehicle", "Rental Date", "Duration", "Cost", "Status"]}
                  keys={["booker", "vehicle", "date", "duration", "cost", "status"]}
                />
